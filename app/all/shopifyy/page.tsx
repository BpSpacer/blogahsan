...
import { simpleShopifyCard } from "@/app/lib/sanityinterface";
import { client } from "@/sanity/lib/client";

export const revalidate = 60;

async function getData() {
  const query = `*[_type == "shopify"]{
    title,
...
export default async function PostsPage() {
  const data = (await getData()) as simpleShopifyCard[];
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((shopify) => (
            <Link
              key={shopify.currentSlug}
              href={`/all/shopify/${shopify.currentSlug}`}
            ...
            >
              <div className="p-4 flex flex-col h-full">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm italic line-clamp-1">
                  {shopify.content?.[0]?.children?.[0]?.text?.slice(0, 100) || "Read more..."}
                </p>

                {shopify.publishedAt && (
                  <div className="mt-4 flex justify-center">
                    <p className="text-sm text-primary dark:text-primary">
                      Published: {new Date(shopify.publishedAt).toISOString().split("T")[0]}
                    </p>
                  </div>
                )}
              </div>
