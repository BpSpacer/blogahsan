...
export default async function PostsPage() {
  const data = (await getData()) as simpleAppCard[];
            ...
            >
              <div className="p-4 flex flex-col h-full">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm italic line-clamp-1">
                  {app.content?.[0]?.children?.[0]?.text?.slice(0, 100) || "Read more..."}
                </p>

                {app.publishedAt && (
                  <div className="mt-4 flex justify-center">
                    <p className="text-sm text-primary dark:text-primary">
                      Published: {new Date(app.publishedAt).toISOString().split("T")[0]}
                    </p>
                  </div>
                )}
              </div>
