...
import { simpleAccountCard } from "@/app/lib/sanityinterface";
...
export default async function PostsPage() {
  const data = (await getData()) as simpleAccountCard[];
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((account) => (
            <Link
              key={account.currentSlug}
              href={`/all/account/${account.currentSlug}`}
            ...
            >
              <div className="p-4 flex flex-col h-full">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm italic line-clamp-1">
                  {account.content?.[0]?.children?.[0]?.text?.slice(0, 100) || "Read more..."}
                </p>

                {account.publishedAt && (
                  <div className="mt-4 flex justify-center">
                    <p className="text-sm text-primary dark:text-primary">
                      Published: {new Date(account.publishedAt).toISOString().split("T")[0]}
                    </p>
                  </div>
                )}
              </div>
